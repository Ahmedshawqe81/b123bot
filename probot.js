...
if (message.content.toLowerCase().startsWith(prefix + `new`)) {
    ...
    message.guild.createChannel(`ticket-${message.author.id}`, "text").then(c => {
        ...
        const embed = new Discord.RichEmbed()
        .setColor(0xCF40FA)
        .addField(`Hey ${message.author.username}!`, `من فضلك قم بشرح سبب فتح التيكت و المساعدين سوف يأتون للمساعده ...`)
        .setTimestamp();
        c.send({ embed: embed });
    }).catch(console.error);
}
if (message.content.toLowerCase().startsWith(prefix + `close`)) {
    if (!message.channel.name.startsWith(`ticket-`)) return message.channel.send(`لا يمكنك استخدام الامر خارج روم التيكت ..`);
 
    message.channel.send(`هل انت متأكد من انك تريد اغلاق الشكوة ؟ في حاله التأكد قم بكتابه p!confirm`)
    .then((m) => {
